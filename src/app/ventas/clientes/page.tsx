...
function roleBadgeClasses(r: string) {
  if (r === "comprador") return "bg-blue-50 text-blue-700 border border-blue-200";
  if (r === "vendedor") return "bg-amber-50 text-amber-700 border border-amber-200";
...
export default function ClientesPage() {
  ...
  const kpiCards: { label: string; value: number; icon: string; bg: string; color: string }[] = [
    { label: "Total Clientes", value: kpis.total, icon: "groups", bg: "bg-blue-100", color: "text-blue-600" },
    { label: "Nuevos Este Mes", value: kpis.newThisMonth, icon: "person_add", bg: "bg-emerald-100", color: "text-emerald-600" },
    { label: "Compradores", value: kpis.compradores, icon: "shopping_bag", bg: "bg-violet-100", color: "text-violet-600" },
    { label: "Vendedores", value: kpis.vendedores, icon: "storefront", bg: "bg-amber-100", color: "text-amber-600" },
  ];
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {kpiCards.map((k) => (
          <div key={k.label} className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-widest font-bold text-cool-grey">{k.label}</p>
      ...
      <div className="flex gap-6">
        {/* Table */}
        <div className={`bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden transition-all ${selectedClient ? "flex-1 min-w-0" : "w-full"}`}>
          {filtered.length === 0 ? (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50">
                    ...
                    <th className="px-4 py-4 text-[10px] uppercase tracking-widest font-bold text-cool-grey hidden lg:table-cell">Email</th>
                    <th className="px-4 py-4 text-[10px] uppercase tracking-widest font-bold text-cool-grey">Rol</th>
                    <th className="px-4 py-4 text-[10px] uppercase tracking-widest font-bold text-cool-grey hidden md:table-cell">Operaciones</th>
                    <th className="px-4 py-4 text-[10px] uppercase tracking-widest font-bold text-cool-grey hidden md:table-cell">Última Actividad</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((c, i) => (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {c.instagram && <p className="text-[11px] text-cool-grey">@{c.instagram}</p>}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4 text-sm">{c.phone || "—"}</td>
                      <td className="px-4 py-4 text-sm text-on-surface-variant hidden lg:table-cell">{c.email || "—"}</td>
                      <td className="px-4 py-4">
                        <span className={`inline-block px-2.5 py-1 rounded-full text-[11px] font-bold ${roleBadgeClasses(c.role)}`}>
                          {roleLabel(c.role)}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-sm text-cool-grey hidden md:table-cell">0</td>
                      <td className="px-4 py-4 text-xs text-cool-grey hidden md:table-cell">{timeAgo(c.updated_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {selectedClient && (
          <div className="w-[360px] flex-shrink-0 hidden lg:block space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <button
                  ...
                  className="text-cool-grey hover:text-on-surface transition-colors"
                >
              ...
              <div className="space-y-3">
                {selectedClient.phone && (
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-base text-cool-grey">phone</span>
                    <span className="text-sm">{selectedClient.phone}</span>
                  </div>
                )}
                {selectedClient.email && (
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-base text-cool-grey">mail</span>
                    <span className="text-sm">{selectedClient.email}</span>
                  </div>
                )}
                {selectedClient.instagram && (
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-base text-cool-grey">photo_camera</span>
                    <span className="text-sm">@{selectedClient.instagram}</span>
                  </div>
                )}
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-base text-cool-grey">calendar_today</span>
              ...
              <button
                ...
                className="w-full mt-5 py-2.5 bg-slate-100 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-200 transition-colors flex items-center justify-center gap-2"
              >
            ...
            {/* Notes Card */}
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <p className="text-[10px] uppercase tracking-widest font-bold text-cool-grey mb-3">Notas</p>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <p className="text-[10px] uppercase tracking-widest font-bold text-cool-grey mb-3">Historial de Operaciones</p>
              <div className="flex flex-col items-center py-6 text-cool-grey">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowModal(false)} className="text-cool-grey hover:text-on-surface transition-colors">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="text-[10px] font-bold text-cool-grey uppercase tracking-widest">Nombre *</label>
                <input
                  required
                  value={form.name}
              ...
              <div>
                <label className="text-[10px] font-bold text-cool-grey uppercase tracking-widest">Email</label>
                <input
                  type="email"
                  value={form.email}
              ...
              <div>
                <label className="text-[10px] font-bold text-cool-grey uppercase tracking-widest">Instagram</label>
                <input
              ...
              <div>
                <label className="text-[10px] font-bold text-cool-grey uppercase tracking-widest mb-2 block">Rol</label>
